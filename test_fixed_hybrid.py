...
def test_fixed_hybrid():
    ...
    print(
        f"GPU Status: {gpu_info.get('used_mb', 0)}MB / {gpu_info.get('total_mb', 0)}MB"
    )
    ...
    try:
        ...
        result = scorer.process_task(
            {"test_data": "qa_validation", "target_fitness": 0.75},
            generation=1,
        ...
        if result.success:
            ...
            print(f"\n=== VALIDATION RESULTS ===")
            print(
                f"Response time <30s: {'✅' if response_time_ok else '❌'} ({execution_time:.1f}s)"
            )
