...
class OllamaSpecialist(ABC):
    ...
    def __init__(self, model_name: str = "qwen3:8b", gpu_enabled: bool = True):
        ...
        logger.info(f"OllamaSpecialist initialized: model={model_name}, GPU={self.gpu_enabled}")
...
class FitnessScorer(OllamaSpecialist):
    ...
    def __init__(self):
        super().__init__(model_name="deepseek-coder:1.3b")  # Use smaller model for testing
    ...
    def _combine_fitness_evaluations(self, evaluations: Dict[str, Any], current_fitness: float) -> float:
    ...
    def _format_hybrid_response(self, evaluations: Dict[str, Any]) -> str:
        ...
        if evaluations.get("qwen3", {}).get("success"):
            ...
            response_parts.append(f"Qwen3 Error: {evaluations['qwen3'].get('error', 'Unknown')}")

        if evaluations.get("ollama", {}).get("success"):
            response_parts.append(f"Ollama: {evaluations['ollama']['response']}")
        elif "ollama" in evaluations:
            response_parts.append(f"Ollama Error: {evaluations['ollama'].get('error', 'Unknown')}")

        return " | ".join(response_parts) if response_parts else "No evaluations available"
    ...
    def evaluate_generation(self, generation: int, current_fitness: float) -> Dict[str, Any]:
        ...
        result = self.process_task(task_data, generation=generation, current_fitness=current_fitness)
        return result.data
...
class TTSHandler(OllamaSpecialist):
    ...
    def _setup_d_drive_cache(self):
        ...
        cache_dirs = [
            'D:\\huggingface_cache', 'D:\\transformers_cache', 'D:\\datasets_cache',
            'D:\\torch_cache', 'D:\\cache', 'D:\\temp', 'D:\\bark_cache'
        ]
    ...
    def _check_bark_availability(self) -> bool:
        ...
        try:
            ...
            logger.info("Bark TTS available with comprehensive PyTorch 2.6+ compatibility")
            return True
    ...
    def _generate_bark_audio(self, text: str, voice_preset: str) -> Optional[Dict[str, Any]]:
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(f"Audio generated successfully: {len(audio_array)} samples")
...
class SwarmCoordinator(OllamaSpecialist):
    ...
    def _aggregate_results(self, results: List[TaskResult]) -> Dict[str, Any]:
        ...
        aggregated = {
            ...
            "gpu_utilization_rate": sum(1 for r in results if r.gpu_utilized) / len(results)
        }
