...
class EvolvableNeuralNet(nn.Module):
    ...
    def __init__(self, input_size: int = 784, hidden_sizes: List[int] = [128, 64], output_size: int = 10):
    ...
    def set_parameters_vector(self, param_vector: np.ndarray):
        ...
        for param in self.parameters():
            ...
            param.data = torch.tensor(new_param.reshape(param_shape), dtype=param.dtype, device=param.device)
    ...
    def mutate(self, mutation_rate: float = 0.1, mutation_strength: float = 0.01):
        ...
        logger.debug(f"Neural network mutated with rate={mutation_rate}, strength={mutation_strength}")
...
class NeuroEvolutionEngine:
    ...
    def __init__(self, population_size: int = 50, gpu_accelerated: bool = True):
        ...
        logger.info(f"NeuroEvolution engine initialized with population_size={population_size}, GPU={self.gpu_accelerated}")
    
    def _initialize_population(self):
        """Initialize population of evolvable neural networks"""
        device = torch.device('cuda' if self.gpu_accelerated and torch.cuda.is_available() else 'cpu')
        ...
        logger.info(f"Population of {self.population_size} neural networks initialized on {device}")
    ...
    def evolve_generation(self) -> float:
        ...
        logger.info(f"Generation {self.metrics.generation_count} completed in {generation_time:.2f}s, best fitness: {best_fitness:.4f}")
    ...
    def dgm_self_modify(self) -> Dict[str, Any]:
        ...
        if modification_confidence < confidence_threshold:
            logger.warning(f"DGM modification confidence {modification_confidence:.3f} below threshold {confidence_threshold}")
            return {"error": "Confidence below threshold", "confidence": modification_confidence}
            ...
            if fitness < threshold:
                ...
                try:
                    if modification_type == "architecture_mutation":
                        ...
                        scale_factor = max(0.5, min(2.0, 1.0 + (threshold - fitness) * 0.1))
                        with torch.no_grad():
        ...
        if modified_networks:
            post_modification_scores = [self._compute_fitness(self.population[i]) for i in modified_networks]
            pre_modification_scores = [fitness_scores[i] for i in modified_networks]

            # Rollback modifications that degraded performance
            for idx, (pre_score, post_score, net_idx) in enumerate(zip(pre_modification_scores, post_modification_scores, modified_networks)):
                if post_score < pre_score * 0.95:  # Allow 5% tolerance
                    logger.warning(f"Rolling back network {net_idx}: fitness degraded {pre_score:.3f} -> {post_score:.3f}")
