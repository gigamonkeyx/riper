...
def run_fallback_tests() -> Dict[str, Any]:
    ...
    for test_func in tests:
        try:
            ...
            logger.info(
                f"{result['test_name']}: {status} ({result['fitness_score']:.3f})"
            )
    ...
    logger.info(
        f"Success Rate: {summary['success_rate']:.1%} ({passed_tests}/{len(tests)})"
    )
    logger.info(
        f"Threshold Met (≥70%): {'✅ YES' if summary['threshold_met'] else '❌ NO'}"
    )
