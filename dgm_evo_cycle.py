...
for generation in range(MAX_GENERATIONS):
    ...
    if (generation + 1) % 10 == 0 or fitness >= FITNESS_TARGET:
        print(f'Gen {generation + 1:3d}: {fitness:.4f} (best: {best_fitness:.4f}) [{gen_time:.2f}s]')
    ...
    if (generation + 1) % HYBRID_ANALYSIS_INTERVAL == 0:
        try:
            ...
            if response.success:
                ...
                print(f'    ✅ Qwen3 analysis completed ({response.execution_time:.1f}s)')
            else:
    ...
    if (generation + 1) % 20 == 0:
        ...
        if 'error' not in modifications:
            mod_count = sum(modifications.values()) if isinstance(modifications, dict) else 0
...
if hybrid_analyses:
    avg_analysis_time = sum(a['execution_time'] for a in hybrid_analyses) / len(hybrid_analyses)
...
try:
    ...
    if result.returncode == 0:
        ...
        print(f'GPU VRAM: {used_mb}MB/{total_mb}MB ({used_mb/1024:.1f}GB/{total_mb/1024:.1f}GB)')
...
print(f'✅ DGM Evolution Cycle: {"SUCCESSFUL" if best_fitness >= FITNESS_TARGET else "PARTIAL SUCCESS"}')
